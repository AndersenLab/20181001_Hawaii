...
# load freezing log spreadsheet
proc_strain_df <- data.table::fread('data/HW_2018_strain_processing.csv') %>%
  dplyr::select(initial_spp_id:notes) %>%
  dplyr::filter(initial_spp_id != "")
  
# filter to Caenorhabditis strains that we attempted to cryopreserve
cryo_df <- df %>%
